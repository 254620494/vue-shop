<template>
  <div class="address">
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) of filterDatalist" :class="{'check':index==currentIndex}" @click="currentIndex=index">
                <dl  >
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-edit" >
                  <a href="javascript:;" class="addr-edit-btn" @click="editItem(item)">
                    <svg class="icon icon-edit"><use xlink:href="#icon-edit"></use></svg>
                  </a>
                </div>
                <div class="addr-opration addr-del" >
                  <a href="javascript:;" class="addr-del-btn">
                    <svg class="icon icon-del" @click="deleteItems(item)"><use xlink:href="#icon-del"></use></svg>
                  </a>
                </div>
              ...
              <li class="addr-new" @click="addItems">
        ...
        <div class="shipping-method-wrap">
          <div class="shipping-method">
            <ul>
              <li :class="{'check': pass==1}" @click="pass=1">
                <div class="name">标准配送</div>
                <div class="price">Free</div>
              </li>
              <li :class="{'check': pass==2}" @click="pass=2" >
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModal" :class="{'md-show':deleteClass}">
          <div class="md-modal-inner">
            ...
            <div class="md-content">
              ...
              <div class="btn-wrap col-2">
                <button class="btn btn--m" id="btnModalConfirm" @click="deleteConfirm" >Yes</button>
   ...
   <!--     <div class="vislay" v-if="addItem"></div> -->
         <div class="md-modal modal-msg md-modal-transition" id="showModalw" :class="{'md-show':editClass}"> 
          <div class="md-modal-inner">
            <div class="md-top">
              <button class="md-close"   @click="editAll('fals')">关闭</button>
            </div>
            <div class="md-content">
              <div class="confirm-tips">
                <div class="md-form-item">
                  ...
                  <div class="md-form-item__content" style="margin-left: 80px;">
                    <div  class="el-input">
                      <input type="text" autocomplete="off" class="md-input__inner"  style="width: 22em;" v-model="name">
                    </div>
                  </div>
                </div>
                <div class="md-form-item">
                  ...
                  <div class="md-form-item__content" style="margin-left: 80px;">
                    <div  class="el-input">
                      <input type="text" autocomplete="off" class="md-input__inner" style="width: 22em;" v-model="street">
                    </div>
                  </div>
                </div>
                <div class="md-form-item">
                  ...
                  <div class="md-form-item__content" style="margin-left: 80px;">
                    <div  class="el-input">
                      <input type="text" autocomplete="off" class="md-input__inner"  style="width: 22em;" v-model="tel">
                    </div>
                  </div>
                </div>
              </div>
              <div class="btn-wrap col-2">
                <button class="btn btn--s" id="btnModalConfirms"  @click="editAll('tru')" >保存</button>
                <button class="btn btn--s btn--red" id="btnModalCancels"  @click="editAll('fals')" >取消</button>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
	  data () {
		...
		mounted () {
		  this.$nextTick(function () {
		    this.getData2();// Code that will run only after the
		    // entire view has been rendered
		  })
		},
		computed : {
				filterDatalist () {
					return this.dataList.slice(0,this.slicePage)
				}
		},
		watch: {
			dataList:function() {
	  ...
	  methods : {
	  	getData2 () {
	  		var _this = this ;
	  		this.$http.get('../static/data/address.json').then(function(res){
	  	...
	  	more () {
	  		if(this.slicePage!=this.dataList.length)
	  		{
	  			this.slicePage=this.dataList.length
	  		}else{
	  			this.slicePage=3
	  		}
	  	},
	  	...
	  	deleteConfirm () {
	  		this.dataList.splice(this.deleteItem,1);
	  	...
	  	editAll (tt) {
	  		...
	  		if(tt=='tru'){
	  			this.dataList[this.editIndex].userName = this.name;
	  			this.dataList[this.editIndex].streetName = this.street;
	  			this.dataList[this.editIndex].tel = this.tel;
	  			this.editClass = false;
	  		};
	  		if(tt == 'fals'){
	  			if(this.editIndex+1==this.dataList.length){
	  				this.dataList.splice(this.editIndex,1)
	  			}
	  			this.editClass = false;
	  		}
	  	}
